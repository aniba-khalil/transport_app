...
export default function GestionBus() {
  ...
  const [message, setMessage] = useState(""); // ⚡ Nouveau état pour afficher les messages
  ...
  const res = await fetch("http://10.128.179.175:5000/api/bus/update", {
    method: "POST",
  ...
  const ajoutbus = async () => {
    ...
    try {
      const res = await fetch("http://10.128.179.175:5000/api/bus/ajouter", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
